...
def Calc_Distance (df, X_Var, X_Center, Importance):
    Hom = 1. / X_Var
...
def ENC(X, y, X0_Center, X0_Var, X1_Center, X1_Var, Importance, train_mode, cut_off, pred_min, pred_max):
    Distance0 = Calc_Distance(X, X0_Var, X0_Center, Importance )
    Distance1 = Calc_Distance(X, X1_Var, X1_Center, Importance )
...
def ENC_Tuning(X_train, y_train):
    ...
    pred, pred_class, cut_off = ENC(X_train,y_train, X0_Center, X0_Var, X1_Center, X1_Var, Importance, True, 0, pred_min, pred_max)
...
class ENCClassifier(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        = ENC_Tuning(self.X_train, self.y_train)
        return
